...
from image_augmentation.general.preprocessing import blending_one_image, blending_images
...
def aug_images_from_xml(cat_name, image_files, xml_files, background_path, cat_aug_path, xml_folder, xml_source,
...
def aug_images_from_image(blending_method, image_files, background_path, cat_aug_path, xml_folder, xml_source,
               cat_name, x_proportion=0.4, y_proportion=0.5, max_num=1000):
    background_images = get_background_images(background_path)
...
def test_single():
    data_path = r"E:\Programming\Python\8_Ganlanz\food_recognition\dataset\自建数据集\5_抽取目标"
    ...
    for dir_ in dirs:
        ...
        print("类别：{}\t有效标注文件数量：{}".format(dir_, len(image_files)))
        aug_images_from_image("", image_files, background_path, cat_aug_path_d, folder, source,cat_name=dir_,
                   x_proportion=x_proportion, y_proportion=y_proportion)
